...
class ProximityEngine:
    ...
    @staticmethod
    def check_proximity_to_locations(
        db: Session,
        business_id: UUID,
        user_lat: float,
        user_lon: float,
    ) -> List[Tuple[LocationProfile, float]]:
        """Return list of (location, distance_km) tuples within any location's geofence."""
        locations = db.query(LocationProfile).filter(
            and_(
    ...
    @staticmethod
    def trigger_proximity_automation(
        db: Session,
        business_id: UUID,
        user_id: UUID,
        ...
        """Log proximity event. Would trigger automation in real system."""
        event = LocationService.log_proximity_event(
            db=db,
        ...
        # In real system, would trigger automations here:
        # - Auto-send SMS/WhatsApp "Hey, you're near us!"
        # - Send proximity-triggered email
    ...
    @staticmethod
    def get_proximity_history(
        db: Session,
        business_id: UUID,
        user_id: UUID,
    ) -> List[dict]:
        """Get proximity detection history for user across all locations."""
        events = db.query(ProximityEvent).filter(
            and_(
                ProximityEvent.business_id == business_id,
