...
| `BusinessModelAgent` | 3 | `bt_business_model_redesigns` — **v2, costed decision.** `model_diagnosis` (how it earns now, margin leaks, every-sale-from-zero?, fragility), `pattern_evaluation` (shortlist scored 0-2 on `BUSINESS_MODEL_PATTERN_AXES`: positioning-fit / margin / retention / execution-difficulty / time-to-cash → apply/later/reject), `canvas` + `canvas_changes` (per block: from→to→why→forces-change-in), `value_equation` (quantified Hormozi, before/after on time-delay + effort), `grand_slam_offer` (bonuses each tied to an objection, guarantee w/ stated worst-case abuse cost), `pricing_architecture` (outcome-named tiers w/ expected mix %, named tactics from `PRICING_PSYCH_TACTICS`), `pricing_migration` (grandfather/phase-in + churn risk + message), `unit_economics_targets` (gross margin, CAC ceiling, payback, LTV:CAC, contribution — each w/ its assumption), `rollout` (change-first / validate-before / sequence), `risks` (likelihood + mitigation). |
| `FOMOEngineAgent` | 4 | `bt_fomo_playbooks` — **v2, desire engine with a conscience.** `lever_selection` (all 7 levers scored 0-2 on `FOMO_LEVER_AXES` incl. **ethical_risk**, chosen bool), `mechanisms` (each + the psychological `trigger`, a *measurable* `anti_fake_guardrail`, and an `honest_alternative` for when it can't be real this cycle), `ethics_review` (the manipulation line for this brand, `never_do` list, the "shown from the inside" test), `activation_sequence` (first mechanism / gating conditions / 90-day ramp), `launch_ritual` (+ what makes it repeatable), `content_hooks` (copy angles reused by GTM), `measurement` (leading vs lagging), `integration_notes` (**each mechanism → the concrete SellIA `fomo` / `fomo_intelligence` endpoint that implements it**, via `FOMO_DOMAIN_MAP`), `risk_matrix` (backfire mode / early warning / kill switch). |
| `GoToMarketAgent` | 5 | `bt_gtm_plans` — **v2, one loop not a wish-list.** `loop_evaluation` (6 loop types scored 0-2 on `GROWTH_LOOP_AXES`: model-fit / margin-supports-it / time-to-compound / defensibility / team-can-run-it → primary/secondary/reject), `primary_growth_loop` (the variable it turns, the `turning_metric` that says it's working, `cycle_time`, why each other loop loses), `channel_plan` (each: hypothesis / first action / **effort estimate** / leading signal / kill signal / role), `lightning_strike` (the one moment + carrying asset + day-by-day + success number), `content_engine` (pillars from the brand story spine + formats + `repurposing_chain` + owner), `funnel` (per stage: the job / asset / metric / top drop-off), `plan_90_days` (milestones w/ owner + metric + `depends_on`), `week_1_actions` (3), `budget_shape` (allocation + the binding constraint), `north_star_metric`, `anti_goals`. Consumes positioning + brand + business-model + fomo via `_stage_context_digest`. *(also fixes a latent bug: `alternative_angles` had no column.)* |
| `RestructuringAgent` | 6 | `bt_restructuring_plans` — **v2, operating system.** `kill` (each w/ sunk-cost rebuttal + what it frees + who pushes back), `keep`/`scale` (w/ risk-if-dropped / first-constraint / prereq), `capability_gaps` (build/buy/borrow per gap), `the_one_hire`, `decision_rights` (one owner + consulted + informed per key decision — no committees), `core_processes` (each: trigger / steps / SLA / **the brand promise it protects** / failure mode), `operating_rhythm` (daily/weekly/monthly/quarterly — purpose / attendees / the decision it produces), `promise_kpis` (proves-which-promise / replaces-vanity / source / baseline), `unit_economics_gate` (thresholds carried from Etapa 3 + the explicit **pause-growth-spend trigger**), `operating_plan_90d` (sequenced w/ `depends_on`), `transition_risks`. |
